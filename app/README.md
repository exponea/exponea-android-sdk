...
Next we should provide our sdk with integration configuration
```kotlin
...
> That applies only if automaticPushNotification is set to `true`
...
Another lonely button. This one will fully anonymize current user by assigning new UUID and resetting any other Ids.
```kotlin
